...
TEST(GRPCCacheService, GetMissingKeyReturnsNotFound) {
  cache::core::ConcurrentStore store(4);
  cache::protocol::grpc::CacheServiceImpl service(store);
  ...
  cache::GetRequest get_request;
  get_request.set_key("missing");
  cache::GetResponse get_response;
  grpc::ClientContext get_context;
  auto get_status = stub->Get(&get_context, get_request, &get_response);
  ASSERT_TRUE(get_status.ok());
  EXPECT_FALSE(get_response.found());
  EXPECT_TRUE(get_response.error().empty());

  server->Shutdown();
}

TEST(GRPCCacheService, EmptyKeyReturnsInvalidArgument) {
  cache::core::ConcurrentStore store(4);
  cache::protocol::grpc::CacheServiceImpl service(store);
  ...
  grpc::ClientContext set_context;
  auto set_status = stub->Set(&set_context, set_request, &set_response);
  EXPECT_EQ(set_status.error_code(), grpc::StatusCode::INVALID_ARGUMENT);
  EXPECT_EQ(set_status.error_message(), "key must not be empty");
  ...
  EXPECT_EQ(get_status.error_message(), "key must not be empty");
